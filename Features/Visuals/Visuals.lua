...
local onNPCsDescendantAdded = LPH_NO_VIRTUALIZE(function(descendant)
	...
	return emplaceObject(
		descendant,
		ModelESP.new("NPC", descendant, string.format("%s", descendant.Name))
	)
end)
...
local onEntitiesChildAdded = LPH_NO_VIRTUALIZE(function(child)
	...
	return emplaceObject(child, MobESP.new("Mob", child,child.Name))
end)
...
function Visuals.init()
	local ents = workspace:WaitForChild("Live")

...
PlayerESP.update = LPH_NO_VIRTUALIZE(function(self)
	...
	if rank then
		tags[#tags + 1] = ESP_GRADE:format(rank)
	else
		tags[#tags + 1] = ESP_GRADE:format("Unknown Rank")
	end

	if Configuration.idToggleValue(identifier, "ShowElement") then
		tags[#tags + 1] = ESP_ELEMENT:format(model:GetAttribute("WeaponClass") or "Unknown Weapon")
	end
